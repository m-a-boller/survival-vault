...
app = FastAPI(title="Survival Vault API", version="0.4.1")
...
def get_shopping_list(grouped: bool = False, db: Session = Depends(get_db)) -> Union[List[models.Item], Dict[str, List[models.Item]]]:
    ...
    if grouped:
        ...
        for item in items:
            cat = str(item.category or "Allgemein")
...
def purchase_item(item_id: int, amount: Optional[float] = None, db: Session = Depends(get_db)) -> models.Item:
...
def read_weapons(db: Session = Depends(get_db)) -> List[models.Weapon]:
    return db.query(models.Weapon).all()

@app.post("/arsenal/weapons", response_model=schemas.Weapon)
def create_weapon(weapon: schemas.WeaponCreate, db: Session = Depends(get_db)) -> models.Weapon:
...
def clean_weapon(weapon_id: int, db: Session = Depends(get_db)) -> models.Weapon:
...
def read_ammunition(db: Session = Depends(get_db)) -> List[models.Ammunition]:
    return db.query(models.Ammunition).all()

@app.post("/arsenal/ammunition", response_model=schemas.Ammunition)
def create_ammunition(ammo: schemas.AmmunitionCreate, db: Session = Depends(get_db)) -> models.Ammunition:
...
def get_arsenal_stats(db: Session = Depends(get_db)) -> List[schemas.ArsenalStats]:
    # Group ammunition by caliber
    ammunition = db.query(models.Ammunition).all()
    stats: Dict[str, float] = {}
    for ammo in ammunition:
        caliber = str(ammo.caliber)
        stats[caliber] = stats.get(caliber, 0.0) + float(ammo.quantity)
    
    # Check which calibers we have weapons for
    weapon_calibers = {str(w.caliber) for w in db.query(models.Weapon).all()}
...
def get_inventory_report(db: Session = Depends(get_db)) -> Response:
...
def read_shelters(db: Session = Depends(get_db)) -> List[models.Shelter]:
    return db.query(models.Shelter).all()

@app.post("/shelters", response_model=schemas.Shelter)
def create_shelter(shelter: schemas.ShelterCreate, db: Session = Depends(get_db)) -> models.Shelter:
...
def update_shelter(shelter_id: int, shelter: schemas.ShelterCreate, db: Session = Depends(get_db)) -> models.Shelter:
...
def delete_shelter(shelter_id: int, db: Session = Depends(get_db)) -> Dict[str, str]:
    db_shelter = db.query(models.Shelter).filter(models.Shelter.id == shelter_id).first()
    if not db_shelter:
        raise HTTPException(status_code=404, detail="Shelter not found")
...
def get_expiry_summary(db: Session = Depends(get_db)) -> List[models.Item]:
...
def read_locations(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)) -> List[models.Location]:
    locations = crud.get_locations(db, skip=skip, limit=limit)
    return locations

@app.post("/locations/", response_model=schemas.Location)
def create_location(location: schemas.LocationCreate, db: Session = Depends(get_db)) -> models.Location:
    return crud.create_location(db=db, location=location)

# Boxes
@app.get("/boxes/", response_model=List[schemas.Box])
def read_boxes(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)) -> List[models.Box]:
    boxes = crud.get_boxes(db, skip=skip, limit=limit)
    return boxes

@app.post("/boxes/", response_model=schemas.Box)
def create_box(box: schemas.BoxCreate, db: Session = Depends(get_db)) -> models.Box:
    return crud.create_box(db=db, box=box)

@app.get("/boxes/{box_id}/qr-url")
def get_box_qr_url(box_id: int, db: Session = Depends(get_db)) -> Dict[str, str]:
...
def get_box_by_uuid(box_uuid: str, db: Session = Depends(get_db)) -> models.Box:
...
def consume_item(item_id: int, amount: float, add_to_list: bool = True, db: Session = Depends(get_db)) -> models.Item:
